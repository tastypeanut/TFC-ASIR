...
sudo docker pull joomla #Hace un pull de los contenedores que se van a usar, manteniendo así la última versión de los mismos.
...
sudo docker run --restart=always --name "$cmsname" --link "$dbname":mysql -p "$cmsport":80 -d joomla #Con éste comando se despliega el contenedor que contiene el CMS y se asocia a la base de datos.
