...
while ($uniqueidbool==false){
	if (exec("sudo /bin/bash /var/www/html/deploy/wordpress/tests.sh 1 $uniquedbname"))
	uniqueidgen();
	else
	$cmsport=exec("sudo /bin/bash /var/www/html/deploy/wordpress/tests.sh 2");
	exec("sudo /bin/bash /var/www/html/deploy/wordpress/script.sh $rootpass $uniquedbname $dbuser $dbpass $uniquecmsname $cmsport");
	echo "
