...
$cms = "Drupal"; //Estas variables guardan información sobre la base de datos en la que se van a insertar diferentes registros cuando un usuario cree un contenedor nuevo.
...
if (!empty($userid) && $userid != 0){ //Esta condición comprueba que ninguna persona que no esté autenticada pueda desplegar un contenedor.
    ...
    if (!empty($rootpass) && !empty($dbname) && !empty($dbuser) && !empty($dbpass) && !empty($domname)){ //Con ésta condición se comprueba que no haya ningún campo vacío.
        while ($uniqueidbool==false){ //Condición que indica que mientras $uniqueidbool sea falso, se ejecute la condición siguiente.
        	if (exec("sudo /bin/bash /opt/cms/deploy/drupal/tests.sh 1 $uniquedbname")){ //Condición que ejecuta el script de checkeo pasándole dos parámetros que requiere dicho script, si se ejecuta bien, significa que el ID ya existe, por lo tanto, vuelve a generar un nuevo ID.
        	   uniqueidgen();
        	   ...
        	   $sql = "INSERT INTO php.containers (user_id, cms, cms_container_name, database_container_name, domain_name) VALUES ( $userid, '$cms', '$uniquecmsname', '$uniquedbname', '$domname')"; //Con esto se forma la sentencia sql a ejecutar, que será la que introducirá todos los valores que se muestran, en la base de datos, para su posterior uso.
            	if ($conn->query($sql) === TRUE) { //Condición que ejecuta la sentencia SQL, y si se ejecuta correctamente, pasa a realizar las acciones siguientes.                    
            		$cmsport=exec("sudo /bin/bash /opt/cms/deploy/drupal/tests.sh 2"); //Esta línea ejecuta la opción 2 del script de tests, que devuelve un número de puerto que no se está utilizando en el rango 49152-65535, y mete dicho valor en una variable para su posterior uso.
            		exec("sudo /bin/bash /opt/cms/deploy/drupal/script.sh $rootpass $uniquedbname $dbuser $dbpass $uniquecmsname $cmsport $domname"); //Ejecuta el script principal que crea todos los contenedores, e introduce las reglas necesarias en el proxy. La ejecución la realiza pasándole todos los valores necesarios al script.
            		...
            		<h3 style='font-weight: bold;'>Yay! Your Drupal website was deployed successfully.</h3>
            		<h4>Follow the instructions below to get started with it.</h4>
            		<hr style='width: 20%; display: block; margin: auto;'>
            		<br/>
            		<p style='text-align: justify'>Once your Drupal website is generated, you will need to register or already have registered the domain name you entered when creating the site. You'll have to create an A registry record binding your domain name (<span style='font-weight: bold;'>$domname</span>) to the following IP: <span style='font-weight: bold;'>54.88.238.75</span>. As a temporary measure, you can add it to your 'hosts' file as shown below in the example:</p>
            		<br/>
            		...
            		<br/>
            		<p style='text-align: justify'>Once the domain name is registered, or the entry is added to your 'hosts' configuration file, you'll be able to access your website simply by entering the domain name in your browser's search bar. The first thing you'll see is the Drupal installation interface. Choose the settings most fitted to your needs, untill you get to the database configuration screen. Here you'll have to plug in the information you entered before, when launching the CMS. Once this is done, continue with the installation until finished, and from there on you're set to go!</p>
            		<br/>
            		<img src='screenshots/setup.png' style='width: 80%; height: auto; display: block; margin: auto; box-shadow: 0 0 20px 0 rgba(0, 0, 0, 0.2), 0 5px 5px 0 rgba(0, 0, 0, 0.24);'/>
            		<br/>
            			...
            			<p style='text-align: left; font-size: 14px;'><span style='font-weight: bold;'>Database Type:</span> MySQL, MaríaDB, Percora Server, or equivalent<p>
            			<p style='text-align: left; font-size: 14px;'><span style='font-weight: bold;'>Database Name:</span> $uniquedbname<p>
            			<p style='text-align: left; font-size: 14px;'><span style='font-weight: bold;'>Database Username:</span> $dbuser<p>
            			<p style='text-align: left; font-size: 14px;'><span style='font-weight: bold;'>Database Password:</span> The database user password that you entered before<p>
            			<p style='text-align: left; font-size: 14px;'><span style='font-weight: bold;'>Host:</span> mysql<p>
            			<p style='text-align: left; font-size: 14px;'><span style='font-weight: bold;'>Port Number:</span> 3306<p>
